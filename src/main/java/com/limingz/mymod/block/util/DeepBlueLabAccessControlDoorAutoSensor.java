...
public class DeepBlueLabAccessControlDoorAutoSensor {
    ...
    private AABB calculateFrontAreaAABB(BlockPos doorPos, Direction facing) {
        ...
        return switch (facing) {
            // 朝东（EAST）：门正面朝东 → 区域应在门的东侧（X+方向）
            case EAST -> new AABB(
                    doorPos.getX() + 1.0D, // 区域左边界（门的右侧，即正面起点）
                    minY,
                    doorCenterZ - halfWidth,
                    doorPos.getX() + 1.0D - depth, // 区域右边界（向东延伸depth格）
                    maxY,
                    doorCenterZ + halfWidth
            );
            // 朝西（WEST）：门正面朝西 → 区域应在门的西侧（X-方向）
            case WEST -> new AABB(
                    doorPos.getX() + depth, // 区域左边界（向西延伸depth格）
                    minY,
                    doorCenterZ - halfWidth,
                    doorPos.getX(), // 区域右边界（门的左侧，即正面起点）
                    maxY,
                    doorCenterZ + halfWidth
            );
            // 朝南（SOUTH）：门正面朝南 → 区域应在门的南侧（Z+方向）
            case SOUTH -> new AABB(
                    doorCenterX - halfWidth,
                    minY,
                    doorPos.getZ() + 1.0D, // 区域前边界（门的南侧，即正面起点）
                    doorCenterX + halfWidth,
                    maxY,
                    doorPos.getZ() + 1.0D - depth // 区域后边界（向南延伸depth格）
            );
            // 朝北（NORTH）：门正面朝北 → 区域应在门的北侧（Z-方向）
            case NORTH -> new AABB(
                    doorCenterX - halfWidth,
                    minY,
                    doorPos.getZ() + depth, // 区域前边界（向北延伸depth格）
                    doorCenterX + halfWidth,
                    maxY,
                    doorPos.getZ() // 区域后边界（门的北侧，即正面起点）
            );
            // 垂直朝向（理论上不会触发，留作兼容）
